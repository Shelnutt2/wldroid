...
class GpuEnvironmentConfigTest {
    ...
    @Test fun commonVars_presentInAllModes() {
        GpuMode.entries.filter { it != GpuMode.AUTO }.forEach { mode ->
            val vars = GpuEnvironmentConfig.buildProcessEnvVars(mode, "wayland-0", "/host/wayland-runtime")
            assertThat(vars).containsEntry("WAYLAND_DISPLAY", "wayland-0")
            assertThat(vars).containsEntry("XDG_RUNTIME_DIR", "/host/wayland-runtime")
    ...
    @Test fun software_setsLibglAlwaysSoftware() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.SOFTWARE, "wayland-0", "/host/wayland-runtime")
    ...
    @Test fun virglGles_setsMesaOverrideAndVtestSock() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.VIRGL_GLES, "wayland-0", "/host/wayland-runtime")
    ...
    @Test fun virglZink_setsZinkVars() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.VIRGL_ZINK, "wayland-0", "/host/wayland-runtime")
    ...
    @Test fun venus_setsVenusVars() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.VENUS, "wayland-0", "/host/wayland-runtime")
    ...
    @Test fun turnipDirect_setsTurnipVars() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.TURNIP_DIRECT, "wayland-0", "/host/wayland-runtime")
        assertThat(vars).containsEntry("GALLIUM_DRIVER", "zink")
    ...
    @Test fun debugEnabled_setsWldroidDebug() {
        val vars = GpuEnvironmentConfig.buildProcessEnvVars(GpuMode.SOFTWARE, "wayland-0", "/host/wayland-runtime", debugEnabled = true)
